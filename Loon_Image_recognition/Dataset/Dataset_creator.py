...
class TripletLossDataset_opencows(data.Dataset):
	def __init__(self, fold, fold_file, split='train', combine=False,known=True,transform=False,img_size=(224,224),suppress_info=True):
		#The root dir for the data
		self.__root = "Cow_images/opencow2020-DatasetNinja"
		#The fold we are currently considering
		...
		#where we've after known or unknown cats,irrelevant if combine is true
		self.__known = known
		#Whether to transform images/labels into pytorch form
		self.__transform = transform

		#directory containing the images
		self.__train_images_dir = os.path.join(self.__root, "identification-train/img")
		self.__test_images_dir = os.path.join(self.__root, "identification-test/img")
		...
		if os.path.exists(self.__fold_file):
			...
			print(f"File path does not exist: {self.__fold_file}")

		#check
		assert self.__fold in self.__folds_dict.keys()

		#Resize the input input image
		self.__img_size = img_size

		#A dictionary storing seperately the list of images filepaths per category
		#For training and testing
		self.__sorted_files = {}

		# A dictionary storing separately the complete lists of filepaths for training and testing
		self.__files = {}
		"""
		Class setup
		"""

		#Create dicts. of categories filepaths
		train_files = {
		...
		if not self.__combine:
			#Create a lst of categories to be removed according to whether we're after known
			#Or unknown classes
			if self.__known:
	...
	"""
	# get the number for this dataset (depending on the split)
	def __len__(self):
	...
	def __getitem(self, index):
		...
		#For sanity checking/debugging, visualize the triplet
		#self.__visualizeTriplet(img_anchor, img_pos, img_neg, label_anchor)
	...
	def printStats(self):
		...
		print(
			f"Fold = {int(self.__fold) + 1}, split = {self.__split}, combine = {self.__combine}, known = {self.__known}")
		print(
			f"Found {self.__num_classes} categories: {len(self.__folds_dict[self.__fold]['known'])} known, {len(self.__folds_dict[self.__fold]['unknown'])} unknown")
