...
"""
File containing input/output utility Functions
"""
...
def allFilesAtDirWithExt(directory, file_extension, full_path=True):
    #make sure we are looking  at a folder
...
#similary, create a sorted list of all folders at a given directory
def allFoldersAtDir(directory, full_path=True):
    ...
    if full_path:
        ...
        folder = [x for x in sorted(os.listdir(directory)) if os.path.isdir(os.path.join(directory, x))]
